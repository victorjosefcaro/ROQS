...
const submitButton = document.getElementById("add-customer-button");
...
function deleteTable(tableIdToDelete) {
  const tableIndex = tables.findIndex((table) => table.id === tableIdToDelete);
...
document
  ...
  .addEventListener("click", function () {
    ...
    if (newTableSeats > 0) {
      ...
      handleNonAjaxError(new Error("Please provide a valid number of seats."));
