document.addEventListener("DOMContentLoaded", function() {
    ...
    reservationForm.addEventListener("submit", function(event) {
        ...
        // Simulate a new customer arriving
        const customerNumber = customerQueue.length + 1;
        const customer = { name, partySize };
    ...
    function findAvailableTable(partySize) {
        return tables.find(table => table.status === "available" && table.seats >= partySize);
    }
    ...
    function updateTableStatus() {
        ...
        tables.forEach(table => {
            ...
            makeAvailableButton.addEventListener("click", function() {
                ...
                const customer = customerQueue.find(customer => customer.table === table.id);
                if (customer) {
                    customer.table = null;
                }
    ...
    makeAvailableButton.addEventListener("click", function() {
        // Make the selected table available
        const tableIdToMakeAvailable = parseInt(prompt("Enter the table number to make available:"));
        ...
        if (table) {
            ...
            // Clear the table assignment for the customer who occupied it
            const customer = customerQueue.find(customer => customer.table === tableIdToMakeAvailable);
            if (customer) {
                customer.table = null;
            }
        }
